    ...
    def _freeze_stages(self):
        if self.frozen >= 0:
            ...
            for p in self.bn1.parameters():
                p.requires_grad=False
        for i in range(1, self.frozen + 1):
...
def _resnet(
    ...
    **kwargs: Any,
    ...
    if pretrained:
        ...
        ostate_dict = model.state_dict()
        for k in ostate_dict.keys():
            if k not in state_dict:
                print("missing key: {}".format(k))
        for k in state_dict.keys():
            if k not in ostate_dict:
                print("unexpected key: {}".format(k))
            else:
                ostate_dict[k] = state_dict[k]
        model.load_state_dict(ostate_dict)
    return model
